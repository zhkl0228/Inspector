...
#include <sys/types.h>
#include "include/unicorn/platform.h"
...
static jclass Unicorn = NULL;
static jclass UnicornException = NULL;
static jclass Long = NULL;
static jmethodID Number_longValue = NULL;
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
   cachedJVM = vm;
    ...
    clazz = (*env)->FindClass(env, "unicorn/Unicorn");
   if ((*env)->ExceptionCheck(env)) {
      return -1;
   }
    if (clazz != NULL) {
        Unicorn = (*env)->NewGlobalRef(env, clazz);
    }
    ...
    if (clazz != NULL) {
        UnicornException = (*env)->NewGlobalRef(env, clazz);
    }
    ...
    Number_longValue = (*env)->GetMethodID(env, clazz, "longValue", "()J");
    ...
    Long = (*env)->NewGlobalRef(env, clazz);
    Long_constructor = (*env)->GetMethodID(env, Long, "<init>", "(J)V");
...
static void cb_hookcode(uc_engine *eng, uint64_t address, uint32_t size, void *user_data) {
   ...
   if((*cachedJVM)->GetEnv(cachedJVM, (void**)&env, JNI_VERSION_1_4) != JNI_OK) {
      if((*cachedJVM)->AttachCurrentThread(cachedJVM, &env, NULL) != JNI_OK) {
         return;
      }
      isAttached = JNI_TRUE;
   }
   (*env)->CallStaticVoidMethod(env, Unicorn, invokeCodeCallbacks, (jlong)eng, (jlong)address, (int)size);
   if (isAttached) {
...
static void cb_hookblock(uc_engine *eng, uint64_t address, uint32_t size, void *user_data) {
   ...
   (*env)->CallStaticVoidMethod(env, Unicorn, invokeBlockCallbacks, (jlong)eng, (jlong)address, (int)size);
   if (isAttached) {
      (*cachedJVM)->DetachCurrentThread(cachedJVM);
   }
}
...
static void cb_hookintr(uc_engine *eng, uint32_t intno, void *user_data) {
   ...
   (*env)->CallStaticVoidMethod(env, Unicorn, invokeInterruptCallbacks, (jlong)eng, (int)intno);
   if (isAttached) {
      (*cachedJVM)->DetachCurrentThread(cachedJVM);
   }
}
...
static uint32_t cb_insn_in(uc_engine *eng, uint32_t port, int size, void *user_data) {
   ...
   res = (uint32_t)(*env)->CallStaticIntMethod(env, Unicorn, invokeInCallbacks, (jlong)eng, (jint)port, (jint)size);
   if (isAttached) {
      (*cachedJVM)->DetachCurrentThread(cachedJVM);
   }
   return res;
...
static void cb_insn_out(uc_engine *eng, uint32_t port, int size, uint32_t value, void *user_data) {
   ...
   (*env)->CallStaticVoidMethod(env, Unicorn, invokeOutCallbacks, (jlong)eng, (jint)port, (jint)size, (jint)value);
   if (isAttached) {
      (*cachedJVM)->DetachCurrentThread(cachedJVM);
   }
}
...
static void cb_insn_syscall(uc_engine *eng, void *user_data) {
   ...
   (*env)->CallStaticVoidMethod(env, Unicorn, invokeSyscallCallbacks, (jlong)eng);
   if (isAttached) {
      (*cachedJVM)->DetachCurrentThread(cachedJVM);
   }
}
...
static void cb_hookmem(uc_engine *eng, uc_mem_type type,
   ...
   switch (type) {
      case UC_MEM_READ:
         (*env)->CallStaticVoidMethod(env, Unicorn, invokeReadCallbacks, (jlong)eng, (jlong)address, (int)size);
         break;
      case UC_MEM_WRITE:
         (*env)->CallStaticVoidMethod(env, Unicorn, invokeWriteCallbacks, (jlong)eng, (jlong)address, (int)size, (jlong)value);
         break;
      case UC_MEM_FETCH:break;
...
static bool cb_eventmem(uc_engine *eng, uc_mem_type type,
   ...
   jboolean res = (*env)->CallStaticBooleanMethod(env, Unicorn, invokeEventMemCallbacks, (jlong)eng, (int)type, (jlong)address, (int)size, (jlong)value);
   if (isAttached) {
      (*cachedJVM)->DetachCurrentThread(cachedJVM);
   }
   return res;
...
static void throwException(JNIEnv *env, uc_err err) {
   //throw exception
   if (err != UC_ERR_OK && UnicornException != NULL) {
      const char *msg = uc_strerror(err);
      (*env)->ThrowNew(env, UnicornException, msg);
   }
}
...
static uc_engine *getEngine(JNIEnv *env, jobject self) {
   ...
   static jfieldID fid;
   if (haveFid == 0 && Unicorn != NULL) {
      //cache the field id
      fid = (*env)->GetFieldID(env, Unicorn, "eng", "J");
      haveFid = 1;
   }
...
JNIEXPORT void JNICALL Java_unicorn_Unicorn_reg_1write_1num
  (JNIEnv *env, jobject self, jint regid, jobject value) {
   ...
   jlong longVal = (*env)->CallLongMethod(env, value, Number_longValue);
...
JNIEXPORT jobject JNICALL Java_unicorn_Unicorn_reg_1read_1num
  (JNIEnv *env, jobject self, jint regid) {
   ...
   jobject result = (*env)->NewObject(env, Long, Long_constructor, longVal);
   if ((*env)->ExceptionCheck(env)) {
      return NULL;
   }
   return result;
