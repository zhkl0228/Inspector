...
public class Unicorn implements UnicornConst, ArmConst, Arm64Const, M68kConst, SparcConst, MipsConst, X86Const {
   ...
   static {
      System.loadLibrary("unicorn");    //loads unicorn.dll  or libunicorn.so
      ...
      if (u != null) {
         ...
         if (funcList != null) {
            for (Tuple p : funcList) {
               ...
               result &= emh.hook(u, type, address, size, value, p.data);
            }
         }
      }
      return result;
 ...
 */
   ...
   public void hook_del(Hook hook) throws UnicornException {
      for (ArrayList<Tuple> l : allLists) {
         for (Iterator<Tuple> iterator = l.iterator(); iterator.hasNext(); ) {
...
*/
   ...
   public void context_free(long handle) {
      free(handle);
   }
